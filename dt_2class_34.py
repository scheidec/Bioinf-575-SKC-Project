...
training[:, 20] = training[:, 20] - 3
testing[:, 20] = testing[:, 20] - 3  
...
dt_old = tree.DecisionTreeClassifier(min_samples_split=20,
                                random_state=99)
dt_old.fit(training[:, 0:20], training[:, 20])
scores = cross_val_score(dt_old, training[:, 0:20], training[:, 20] , cv=5)
...
plt.title('Receiver operating characteristic dt_2class_34')
