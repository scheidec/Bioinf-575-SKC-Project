...
idx_max = np.argmax(scores_all, axis = 0)
...
plt.ylabel('C')
plt.colorbar()
plt.xticks(np.arange(len(gamma_range)), np.log10(gamma_range), rotation=45)
plt.yticks(np.arange(len(C_range)), np.log10(C_range))
